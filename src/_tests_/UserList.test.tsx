...
describe("UserAccordion", () => {
  test("renders user login", () => {
    render(<UserAccordion user={mockUser} isOpen={false} onToggle={jest.fn()} />);
    expect(screen.getByText("mockuser")).toBeInTheDocument();
  });

  test("calls onToggle when clicked", () => {
    const onToggle = jest.fn();
    render(<UserAccordion user={mockUser} isOpen={false} onToggle={onToggle} />);
  ...
  test("shows repo list when open", () => {
    render(<UserAccordion user={mockUser} isOpen={true} onToggle={jest.fn()} />);
